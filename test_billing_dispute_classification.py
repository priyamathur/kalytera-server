...
def test_billing_dispute_classification():
    ...
    print("\n🎯 Expected Classification Analysis:")
    print(f"Primary Failure Mode: TOOL_FAILURE")
    print(f"Reasoning: Agent cannot complete billing lookup due to API/system failure")
    print(f"Accuracy Score: ~0.8 (agent is honest about the limitation)")
    print(f"Goal Alignment: ~0.3 (cannot fulfill user's actual need)")
    print(f"Decision Quality: ~0.6 (appropriate escalation offered)")
    print(f"Completeness: ~0.2 (cannot provide requested information)")
    print(f"Overall Score: ~0.5 (significant failure due to system issue)")
    
    # Demonstrate specialized billing criteria
    print(f"\n📋 Billing-Specific Criteria Applied:")
    print(f"- System cannot access billing details (critical failure)")
    print(f"- No account verification bypass (good security practice)")
    print(f"- Appropriate escalation offered (good process)")
    print(f"- Honest communication about limitation (builds trust)")
    
    print(f"\n✅ This scenario perfectly demonstrates:")
    print(f"  1. Tool failure classification for system outages")
    print(f"  2. Impact on user goal fulfillment")
    print(f"  3. Proper agent behavior during technical issues")
    print(f"  4. Billing-specific evaluation criteria")
...
def test_additional_failure_modes():
    ...
    print(f"✅ Each failure mode has clear definition and examples")
    print(f"✅ Prompts provide specific guidance for classification")
...
def demonstrate_judge_capabilities():
    ...
    print(f"  - Comprehensive evaluation framework covering all failure modes")
    print(f"  - Business-focused scoring that predicts customer satisfaction")
    print(f"  - Context-aware analysis using conversation history")
    print(f"  - Industry-specific evaluation criteria (fintech, SaaS, etc.)")
    
    print(f"\n📈 Business Value:")
    print(f"  - Identifies exact failure points for agent improvement")
    print(f"  - Provides quantitative quality metrics for optimization")
    print(f"  - Enables continuous quality monitoring without manual review")
    print(f"  - Generates actionable insights for product development")
...
if __name__ == "__main__":
    ...
    print(f"✅ Core IP: Sophisticated evaluation prompts and failure taxonomy")
    print(f"✅ Production ready: Batch processing and background jobs")
    print(f"✅ Validated: Billing disputes classified as tool_failure at step 3")
    print(f"✅ Scalable: Handles any agent framework with standardized scoring")